...
glam = function(pars) {
  ...
  g_mat = matrix(0, nrow = n_years, ncol = n_ages)
  for (y in 1:(n_years - 1)) {
    for (a in 1:(n_ages - 1)) {
      g_mat[y, a] = log(wa[y + 1, a + 1]) - log(wa[y, a])
    }
  }
  # g_mat[g_mat < 0] = 0 # replace negative values with 0, assume instantaneous growth rate can't be negative ** this doesn't work
  ...
  if (gill_fleet) {
    ...
    ct_gill_tot = pa_gill = biomass_gill = 0
    ct_gill = matrix(0, nrow = n_years, ncol = n_ages)
  }
  # recreational fleet
  if (rec_fleet) {
  ...
  ## 8. Report Section ####
  out = list()
  out$nage = rowSums(nage)
  out$sp_biomass = rowSums(sp_biomass)
  out$ct_trap = rowSums(ct_trap)
  out$biomass_trap = rowSums(biomass_trap)
  out$recr = recr
  out$sel_trap = sel_trap
  out$pa_trap = pa_trap
  out$resid_pa_trap = resid_pa_trap
  out$M = M
  out$FM_trap = FM_trap
  out$q_trap = q_trap
  if(gill_fleet) {
    out$ct_gill = rowSums(ct_gill)
    out$biomass_gill = rowSums(biomass_gill)
    out$sel_gill = sel_gill
    out$pa_gill = pa_gill
    out$resid_pa_gill = resid_pa_gill
    out$FM_gill = FM_gill
    out$q_gill = q_gill
  }
